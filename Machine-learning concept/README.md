...
## 不完全监督(incomplete)
只有训练集得一个子集是有标签的，其他数据则没有标签。
主动学习和半监督学习。
主动学习：
向人类查询未标注数据的ground truth。目标是最小化查询的数量。

半监督学习的最主要假设：数据分布中有可以挖掘的信息。
...
能够对每个不同的参数调整不同的学习率，对频变化的参数以更小的步长进行更新，而稀疏的参数以更大的步长进行更新。与SGD的核心区别在于计算更新步长时，增加了分母：梯度平方累计和的平方根，该项能够积累各个参数gt的历史梯度平方，频繁更新的梯度，则积累的分母项逐渐偏大，那么更新的步长相对变小。
2. RMSProp
RMSProp结合梯度平方的指数移动平均数来调节学习率的变化



## 2020-4-17
1. 在确定初始学习率的时候，从一个很小的值（e.g. 1e-7）开始，然后每一步指数增大学习率（例如1.05倍）进行训练。训练几百步应该能观察到损失函数随训练步数呈对勾形，选择损失下降最快的那一段学习率即可。