...
def generator_batch_multitask_total_deepidv3(data_list, nbr_class_one=250, nbr_class_two=7, nbr_class_three=1000, batch_size=32, return_label=True,
                    crop_method=center_crop, scale_ratio=1.0, random_scale=False,
                    preprocess=False, img_width=299, img_height=299, shuffle=True,
                    save_to_dir=None, augment=False):
    '''
    ...
    while True:
        ...
        if return_label:
            yield ([X_batch], [Y_batch_one,Y_batch_one,Y_batch_one,Y_batch_one, Y_batch_two,Y_batch_two,Y_batch_two,Y_batch_two, Y_batch_three, Y_batch_three, Y_batch_three, Y_batch_three])
        else:
            yield X_batch
...
def generator_batch_multitask_id_deepidv3(data_list, nbr_class_one=250, nbr_class_two=7, batch_size=32, return_label=True,
                    crop_method=center_crop, scale_ratio=1.0, random_scale=False,
                    preprocess=False, img_width=299, img_height=299, shuffle=True,
                    save_to_dir=None, augment=False):
    '''
    ...
    while True:
        ...
        if return_label:
            yield ([X_batch], [Y_batch_one,Y_batch_one,Y_batch_one,Y_batch_one])
        else:
            yield X_batch

def generator_batch_multitask_id_mutihot(data_list, nbr_class_one=250, nbr_class_two=7, nbr_output=256, batch_size=32, return_label=True,
                    crop_method=center_crop, scale_ratio=1.0, random_scale=False,
                    preprocess=False, img_width=299, img_height=299, shuffle=True,
                    save_to_dir=None, augment=False):
    '''
    ...
    while True:
        ...
        X_batch = np.zeros((current_batch_size, img_width, img_height, 3))
        Y_batch_one = np.zeros((current_batch_size, nbr_output))
        ...
        for i in range(current_index, current_index + current_batch_size):
            ...
            if return_label:
                label_one = int(line[-3])%nbr_class_one
                label1 = (int)(math.sqrt(2.0 * label_one + 0.25) + 1.0)#label_one/nbr_output
                label2 = label_one-(((label1-1)*label1)/2)#label_one%nbr_output
                Y_batch_one[i - current_index, label1] = 1
                Y_batch_one[i - current_index, label2] = 1
...
def generator_batch_multitask_id_MGN(data_list, nbr_class_one=250, nbr_class_two=7, batch_size=32, return_label=True,
                    crop_method=center_crop, scale_ratio=1.0, random_scale=False,
                    preprocess=False, img_width=299, img_height=299, shuffle=True,
                    save_to_dir=None, augment=False):
    '''
    ...
    while True:
        ...
        if return_label:
            yield ([X_batch], [Y_batch_one,Y_batch_one,Y_batch_one,Y_batch_one,Y_batch_one,Y_batch_one,Y_batch_one,Y_batch_one,Y_batch_one])
        else:
            yield X_batch

def generator_batch_multitask_id_PCB(data_list, nbr_class_one=250, nbr_class_two=7, batch_size=32, return_label=True,
                    crop_method=center_crop, scale_ratio=1.0, random_scale=False,
                    preprocess=False, img_width=299, img_height=299, shuffle=True,
                    save_to_dir=None, augment=False):
    '''
    ...
    while True:
        ...
        if return_label:
            yield ([X_batch], [Y_batch_one,Y_batch_one,Y_batch_one,Y_batch_one,Y_batch_one])
        else:
            yield X_batch

def generator_batch_distilling(data_list, feature_map1, feature_map2, mode = 'train', nbr_class_one=250, nbr_class_two=7, batch_size=32, return_label=True,
                    crop_method=center_crop, scale_ratio=1.0, random_scale=False,
                    preprocess=False, img_width=299, img_height=299, shuffle=True,
                    save_to_dir=None, augment=False):
    '''
    ...
    while True:
        ...
        X_batch = np.zeros((current_batch_size, img_width, img_height, 3))
        Y_batch_one = np.zeros((current_batch_size, 1024))
        Y_batch_two = np.zeros((current_batch_size, 256))
        ...
        for i in range(current_index, current_index + current_batch_size):
            ...
            if return_label:
                label_one = line[1]
                Y_batch_one[i - current_index] = feature_map1[label_one]
                Y_batch_two[i - current_index] = feature_map2[label_one]
...
def generator_batch_distilling1(data_list, feature_map1, feature_map2, mode = 'train', nbr_class_one=250, nbr_class_two=7, batch_size=32, return_label=True,
                    crop_method=center_crop, scale_ratio=1.0, random_scale=False,
                    preprocess=False, img_width=299, img_height=299, shuffle=True,
                    save_to_dir=None, augment=False):
    '''
    ...
    while True:
        ...
        X_batch = np.zeros((current_batch_size, img_width, img_height, 3))
        Y_batch_one = np.zeros((current_batch_size, 1280))
        Y_batch_two = np.zeros((current_batch_size, 256))
        ...
        for i in range(current_index, current_index + current_batch_size):
            ...
            if return_label:
                label_one = line[1]
                Y_batch_one[i - current_index][0:1024] = feature_map1[label_one]
                Y_batch_one[i - current_index][1024:] = feature_map2[label_one]
...
def generator_batch_triplet_id(data_list, dic_data_list, nbr_class_one=250, nbr_class_two=7,
                    batch_size=32, return_label=True, mode='train',
                    crop_method=center_crop, scale_ratio=1.0, random_scale=False,
                    img_width=299, img_height=299, shuffle=True,
                    save_to_dir=None, save_network_input=None, augment=False,hard_data_mining = False, hard_data_dic_neg={},hard_data_dic_pos={}):
    '''
    A generator that yields a batch of ([anchor, positive, negative], [class_one, class_two, pseudo_label]).
    ...
    while True:
        ...
        for i in range(current_index, current_index + current_batch_size):
            ...
            if mode == 'train':
                ...
                positive_list.remove(anchor_path)
                positive_path = random.choice(positive_list)
                if hard_data_mining and hard_data_dic_pos.has_key(anchor_path):
                    positive_path = random.choice(hard_data_dic_pos[anchor_path])
                positive = scale_byRatio(positive_path, ratio=scale_ratio, return_width=img_width,
                                    crop_method=crop_method)

                negative_vehicleID_list = dic[modelID][colorID].keys()[:]
                negative_vehicleID_list.remove(vehicleID)
                if hard_data_mining and hard_data_dic_neg.has_key(anchor_path):
                    negative_vehicleID_list = hard_data_dic_neg[anchor_path]
                    #print('hard nagetive data mining')
                
                assert negative_vehicleID_list !=[ ], 'vehicleID_list is [ ], {}'.format(dic[modelID][colorID].keys())
                if not (hard_data_mining and hard_data_dic_neg.has_key(anchor_path)):
                    negative_vehicleID = random.choice(negative_vehicleID_list)
                    negative_path = random.choice(dic[modelID][colorID][negative_vehicleID])
                else:
                    negative_path = random.choice(negative_vehicleID_list)
                negative = scale_byRatio(negative_path, ratio=scale_ratio, return_width=img_width,
                                    crop_method=crop_method)
...
def generator_batch_triplet_id_online_v2(data_list, dic_data_list, nbr_class_one=250, nbr_class_two=7,
                               batch_size=32, return_label=True, mode='train',
                               crop_method=center_crop, scale_ratio=1.0, random_scale=False,
                               img_width=299, img_height=299, shuffle=True,
                               save_to_dir=None, save_network_input=None, augment=False, hard_data_mining=False,
                               hard_data_dic_neg={}, hard_data_dic_pos={}, second_idx=10000):
    '''
    A generator that yields a batch of ([anchor, positive, negative], [class_one, class_two, pseudo_label]).
    ...
    while True:
        ...
        for i in range(current_index, current_index + current_batch_size):
            ...
            if mode == 'train':
                ...
                assert len(
                    dic[modelID][colorID][vehicleID]) > 1, 'vehicleID: {} has only ONE image! The list is  {}'.format(
                    vehicleID, dic[modelID][colorID][vehicleID])
            ...
            if return_label:
                if (int(line[-3])) < second_idx:
                    label_one = int(line[-3]) % nbr_class_one
                # label_two = int(line[-1])
                    Y_batch_one[i - current_index, label_one] = 1
                    Y_batch_two[i - current_index,:] = np.ones_like(Y_batch_two[i - current_index,:]) * (-0.01)
                else:
                    label_one = int(line[-3]) % nbr_class_two
                    Y_batch_two[i - current_index, label_one] = 1
                    Y_batch_one[i - current_index,:] = np.ones_like(Y_batch_one[i - current_index,:]) * (-0.01)
                #else:
                # Y_batch_two[i - current_index, label_two] = 1
...
def generator_batch_triplet_id_online(data_list, dic_data_list, nbr_class_one=250, nbr_class_two=7,
                               batch_size=32, return_label=True, mode='train',
                               crop_method=center_crop, scale_ratio=1.0, random_scale=False,
                               img_width=299, img_height=299, shuffle=True,
                               save_to_dir=None, save_network_input=None, augment=False, hard_data_mining=False,
                               hard_data_dic_neg={}, hard_data_dic_pos={}, online_data_list=[]):
    if shuffle:
        random.shuffle(data_list)
    ...
    while True:
        ...
        X_anchor = np.zeros((current_batch_size, img_width, img_height, 3))
        X_positive = np.zeros((current_batch_size, img_width, img_height, 3))
        X_positive_1 = np.zeros((current_batch_size, img_width, img_height, 3))
        X_negative = np.zeros((current_batch_size, img_width, img_height, 3))
        ...
        for i in range(current_index, current_index + current_batch_size):
            ...
            if mode == 'train':
                ...
                # copy a list of image paths with same vehicleID
                positive_path = random.choice(online_data_list).strip().split(' ')[0]
                positive = scale_byRatio(positive_path, ratio=scale_ratio, return_width=img_width,
                                         crop_method=crop_method)

                negative_path = random.choice(online_data_list).strip().split(' ')[0]
                negative = scale_byRatio(negative_path, ratio=scale_ratio, return_width=img_width,
                                         crop_method=crop_method)
        ...
        if augment:
            ...
            X_negative = X_negative.astype(np.uint8)
            X_anchor = seq.augment_images(X_anchor)
            X_positive_1 = seq.augment_images(X_positive)
            X_positive = seq.augment_images(X_positive)
            X_negative = seq.augment_images(X_negative)
        ...
        if save_to_dir:
            for i in range(current_index, current_index + current_batch_size):
                tmp_path = data_list[i].strip().split(' ')[0]
                basedir = tmp_path.split(os.sep)[-2:]
                image_name = 'a_'.join(basedir)
                image_name_p1 = 'p2_'.join(basedir)
                image_name_p2 = 'p1_'.join(basedir)
                img_to_save_path = os.path.join(save_to_dir, image_name)
                img_to_save_path_p1 = os.path.join(save_to_dir, image_name_p1)
                img_to_save_path_p2 = os.path.join(save_to_dir, image_name_p2)
                img = array_to_img(X_anchor[i - current_index])
                img_p = array_to_img(X_positive[i - current_index])
                img_p2 = array_to_img(X_positive_1[i - current_index])
                img.save(img_to_save_path)
                img_p.save(img_to_save_path_p1)
                img_p2.save(img_to_save_path_p2)

        X_anchor = X_anchor.astype(np.float64)
        X_positive = X_positive.astype(np.float64)
        X_positive_1 = X_positive_1.astype(np.float64)
        X_negative = X_negative.astype(np.float64)
        X_anchor = preprocess_input(X_anchor)
        X_positive = preprocess_input(X_positive)
        X_positive_1 = preprocess_input(X_positive_1)
        X_negative = preprocess_input(X_negative)
        ...
        if return_label:
            yield ([X_anchor, X_positive, X_positive_1, X_negative], [Y_batch_one, Y_batch_fake, Y_batch_fake])
        else:
            if mode == 'feature_extraction':
                yield X_anchor
            else:
                yield [X_anchor, X_positive, X_positive_1, X_negative]
